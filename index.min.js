...
function wrapper(my) {
    ...
    function go(root) {
        !0 === my.metadata && (metadata = path.join(root, ".metadata", path.sep)), require("mongodb-legacy").MongoClient.connect(my.uri, my.options, function(err, _db) {
            function next(err) {
                if (err) return logger("db close"), db.close(), callback(err);
                discriminator(db, root, metadata, parser, function(err) {
                    logger("db close"), _db.close(), callback(err);
                });
            }
            if (logger("db open"), err) return callback(err);
            var db = _db.db();
            return !0 === my.drop ? (logger("drop database"), db.dropDatabase(next)) : my.dropCollections ? (logger("drop collections"), 
            !0 === Array.isArray(my.dropCollections) ? someCollections(db, my.dropCollections, next) : db.collections(function(err, collections) {
    ...
    if ("function" == typeof my.parser) parser = my.parser; else switch (my.parser.toLowerCase()) {
      case "bson":
        BSON = require("bson"), parser = fromBson;
        break;
